...
| Six-area operator dashboard and human user administration | Implemented; Verified locally | [dashboard guide](./dashboard.md), [browser tests](../tests/), [live adapter smoke](../scripts/verify-dashboard-live.ts) | Per-principal login, all six product areas, atomic Add User, and logout pass against real Bun/SQLite through the adapter; exact release deployment evidence is pending. |
