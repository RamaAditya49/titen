...
| Fifteen-area operator dashboard, scoped access, and human administration | Implemented; Verified locally; Verified live | [dashboard guide](./dashboard.md), [browser tests](../tests/), [live adapter smoke](../scripts/verify-dashboard-live.ts), [0.8.6 delivery evidence](./specs/done/2026-08-16-level6-dashboard-release.md#delivery-evidence) | All fifteen capability-gated areas, workspace/grant isolation, model diagnostics, atomic Add User, sealed sessions, package root routing, and explicit audited Atlas administrator mode pass across Bun, D1, the loopback adapter, npm, and production Cloudflare. |
...
| Real Cloudflare deployment; live Vectorize/Workers AI | Implemented; Verified live | [Cloudflare guide](./deployment/cloudflare.md), [0.5.5 delivery evidence](./specs/done/2026-08-01-ponytail-zero.md#delivery-evidence) | A maintainer-operated release stack proves native bindings, scoped vector retrieval, bounded repair, Cron, authorization, persistence, and rollback. Account-specific identifiers and evidence stay outside public artifacts. Each deployment needs its own readiness, drain, query, and rollback smoke. |
| Provisioned VPS systemd ingress | Rootless Quadlet/Podman verified; private-ingress activation is operator-controlled | [VPS guide](./deployment/vps.md), [secure ingress guide](./deployment/secure-ingress.md), [0.5.5 delivery evidence](./specs/done/2026-08-01-ponytail-zero.md#delivery-evidence) | Exact 0.5.5 API and dashboard survive systemd-user restart on `benchmark-host` at schema 21; both listeners are loopback-only. Copyable Tailscale Serve and Cloudflare Tunnel plus Access paths are documented, but neither is claimed active. |
