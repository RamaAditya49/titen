...
| Six-area operator dashboard and human user administration | Implemented; Verified locally; Verified live | [dashboard guide](./dashboard.md), [browser tests](../tests/), [live adapter smoke](../scripts/verify-dashboard-live.ts), [0.5.2 delivery evidence](./specs/done/2026-08-01-dashboard-product-map-session-release.md) | All six areas, login, atomic Add User, and logout pass in the exact-image restored-data canary on `rama-tuf`; production login, capability denial, restart invalidation, and Memories also pass. |
...
| Provisioned VPS systemd ingress | Rootless Quadlet/Podman verified; private-ingress activation is operator-controlled | [VPS guide](./deployment/vps.md), [secure ingress guide](./deployment/secure-ingress.md) | Exact 0.5.2 API and session dashboard survive systemd-user restart on `rama-tuf`; both listeners are loopback-only. Copyable Tailscale Serve and Cloudflare Tunnel plus Access paths are documented, but neither is claimed active. |
