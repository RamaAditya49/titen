...
| Canonical recallable-memory federation | Bounded observation ingestion implemented; broader types planned | [FRD federation feature](./FRD.md#13-signed-federation-event-exchange-and-planned-memory-federation), [collaboration boundary](./architecture/collaboration.md#signed-federation-event-exchange) | Explicit destination policy can ingest eligible observation events with provenance, identity mapping, destination trust/visibility and indexing outbox; claims and other events remain transport-only. |
