...
| Six-area operator dashboard and human user administration | Implemented; Verified locally; Verified live | [dashboard guide](./dashboard.md), [browser tests](../tests/), [live adapter smoke](../scripts/verify-dashboard-live.ts), [0.5.3 delivery evidence](./specs/done/2026-08-01-dashboard-password-accounts-release.md) | Default `owner`, forced first-login password change, all six areas, atomic Add User, and logout pass in the exact-image restored-data canary on `rama-tuf`; production login, capability denial, and restart invalidation also pass. |
