...
| Six-area operator dashboard and human user administration | Implemented; Verified locally; Verified live | [dashboard guide](./dashboard.md), [browser tests](../tests/), [live adapter smoke](../scripts/verify-dashboard-live.ts), [Cloudflare live evidence](./specs/done/2026-08-01-cloudflare-live-production.md) | Default `owner`, forced first-login password change, all six areas, atomic Add User, and logout pass against both the `rama-tuf` canary and the live Cloudflare API through the loopback adapter. |
...
| Real Cloudflare deployment; live Vectorize/Workers AI | Implemented; Verified live | [Cloudflare guide](./deployment/cloudflare.md), [delivery evidence](./specs/done/2026-08-01-cloudflare-live-production.md) | The retained Rama Digital `titen-test-*` stack proves the native bindings, scoped semantic path, Cron, auth, persistence, and rollback. It is isolated test production, not customer cutover or extraction-model activation. |
| Provisioned VPS systemd ingress | Rootless Quadlet/Podman verified; private-ingress activation is operator-controlled | [VPS guide](./deployment/vps.md), [secure ingress guide](./deployment/secure-ingress.md) | Exact 0.5.3 API and session dashboard survive systemd-user restart on `rama-tuf`; both listeners are loopback-only. Copyable Tailscale Serve and Cloudflare Tunnel plus Access paths are documented, but neither is claimed active. |
