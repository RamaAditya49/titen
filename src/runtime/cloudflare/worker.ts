...
export interface Env {
  ...
  TITEN_EXTRACT_GATE_LANES?: string;
  TITEN_EXTRACT_GATE_LINK_MIN_CONFIDENCE?: string;
...
function extraction(env: Env): ReturnType<typeof configureHttpExtraction> {
  const configured = configureHttpExtraction({
    ...
    gateLanes: env.TITEN_EXTRACT_GATE_LANES,
    gateLinkMinConfidence: env.TITEN_EXTRACT_GATE_LINK_MIN_CONFIDENCE === undefined
      ? undefined
      : Number(env.TITEN_EXTRACT_GATE_LINK_MIN_CONFIDENCE),
  });
  if (
    ...
    const app = createApp({
      ...
      modelConfiguration: {
        extraction: {
          ...
          modelFingerprint: env.TITEN_EXTRACT_MODEL_FINGERPRINT,
          responseMode: env.TITEN_EXTRACT_RESPONSE_MODE,
          timeoutMs: env.TITEN_EXTRACT_TIMEOUT_MS === undefined ? undefined : Number(env.TITEN_EXTRACT_TIMEOUT_MS),
          apiKeySet: Boolean(env.TITEN_EXTRACT_API_KEY),
        },
        embedding: {
          model: env.TITEN_EMBED_MODEL,
          dimensions: env.TITEN_EMBED_DIMS === undefined ? undefined : Number(env.TITEN_EMBED_DIMS),
          revision: env.TITEN_EMBED_REVISION,
          profile: env.TITEN_EMBED_PROFILE,
          minimumCosine: env.TITEN_EMBED_MIN_COSINE === undefined ? undefined : Number(env.TITEN_EMBED_MIN_COSINE),
          apiKeySet: false,
        },
      },
