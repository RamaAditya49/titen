...
| `src/runtime/bun/server.ts:86` | Serve one process with one synchronous database handle | Throughput stays bound to one core | An equivalent-quality, durability-preserving small-team workload misses its accepted objective; profile before choosing workers or read replicas (#123) |
...
| `src/core/db.ts:41` | Use one global SQL parameter chunk size | Wide lists multiply expensive statement round trips | A bounded query stays dominated by round trips after its SQL shape is fixed |
